# Cache Showcase — Node.js + Redis + Postgres

A small backend project built to demonstrate practical caching patterns you'd
actually use in production, not just "add Redis and call `.get()`."

## What this demonstrates

- **Cache-aside (lazy loading)** for reads: check Redis first, fall back to
  Postgres on a miss, populate the cache with a TTL.
- **Explicit cache invalidation** on writes, so updates never serve stale data.
...
- **Write-through variant** for comparison, with a documented tradeoff (see below).
- **Observability**: a `/metrics` endpoint reporting cache hit/miss ratio and
  lock-wait count, because "trust me, it's cached" isn't a substitute for data.
...
## Why cache-aside instead of write-through for the primary path
...
Rule of thumb used here: pick write-through when reads-after-write are common
...
- Move lock/metrics logic into a reusable npm-style module if this pattern
  were used across many resource types.
